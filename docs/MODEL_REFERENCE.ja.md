...
計算 curve が現実から離れたときは、外部補正・surrogate 補正・MVEM への逃避ではなく、数理 model を見直して不足している物理現象を組み込み、更新後の ODE 系を解き直す方針を取ります。
...
- [USER_MANUAL.ja.md](USER_MANUAL.ja.md): CLI 操作、artifact の見方、reference sweep の読み方
- [SURROGATE_GUIDE.ja.md](SURROGATE_GUIDE.ja.md): 過去に使っていた `surrogate` 用語の整理メモ。現在の CLI sweep の既定経路では採用しません。
- [MODEL_REFERENCE.md](MODEL_REFERENCE.md): この model reference の英語版
- [../ENGINE_MODEL_WORKLOG.md](../ENGINE_MODEL_WORKLOG.md): 実装変更の時系列ログ


## 文書と code の traceability

保守対象の CLI workflow は、次の責務分離で実装しています。

- `src/simulator.rs`: ODE state の積分、algebraic closure の評価、`p-V` / `p-theta` / `T-S` の再構成、torque bookkeeping
- `src/cli.rs`: WOT operating-point policy の選択、absorber model による speed hold、brake torque 中心の artifact 出力
- `src/config.rs`: default parameter と YAML schema
- `config/reference_na_i4.yaml`: manual が参照する高回転型自然吸気 2.0 L の参照 calibration

CLI artifact で dyno 的な brake quantity として出している値は

```math
\tau_{brake} = \tau_{net} + \tau_{load}
```

であり、`tau_net` 単独よりも speed-hold dynamometer の読み方に対応します。

## 出典の anchor

現在の参照 calibration と文書は、主に次の source family を anchor にしています。

1. 高回転型自然吸気 2.0 L SI engine の公開 brochure / catalog にある power / torque envelope。
2. Heywood, *Internal Combustion Engine Fundamentals* (2nd ed.)。
3. Stone, *Introduction to Internal Combustion Engines* (4th ed.)。
4. Woschni 系の cylinder heat transfer 文献と、single-zone heat-release / pressure reconstruction の標準文献。
...
## display-oriented single-zone `p-V` / `p-theta`
...
表示上の closed-cycle pressure solve は `180 degCA .. EVO` に対して上の式を使います。
`gamma(theta)` は未燃混合気側 `mixture_gamma` から燃焼後 `gamma_expansion` へ遷移させ、
`dQ_loss/dtheta` は burn-rate と post-burn window に分配した cycle heat-loss estimate です。
吸気・排気 stroke は intake / exhaust boundary pressure に対する display 用接続です。
...
- compact reduced-order state と filling-and-emptying
...
- display-oriented single-zone `p-V` pressure solve

ここでいう `surrogate` の意味や、`closure`、`map`、display-model との違いは
[SURROGATE_GUIDE.ja.md](SURROGATE_GUIDE.ja.md) を参照してください。
...
| CLI sweep orchestration と output 書き出し | `src/cli.rs` |
| `p-V` / `p-theta` / `T-S` artifact 再構成 | `src/simulator.rs`, `src/cli.rs` |
