...
pub(super) fn annunciator(
    ...
    let desired = egui::vec2(98.0, 30.0);
    ...
    let lamp_center = egui::pos2(rect.left() + 14.0, rect.center().y);
...
pub(super) fn digital_readout(
    ...
    theme.instrument_frame(accent).show(ui, |ui| {
        let card_width = width.clamp(128.0, 300.0);
        ...
        ui.horizontal(|ui| {
            ui.allocate_ui_with_layout(
                egui::vec2(value_slot_width, 24.0),
...
pub(super) fn linear_meter(ui: &mut egui::Ui, theme: DashboardTheme, spec: LinearMeterSpec<'_>) {
    let card_width = spec.width.clamp(160.0, 300.0);
    ...
    theme.instrument_frame(spec.accent).show(ui, |ui| {
        ...
        let desired = egui::vec2((card_width - 20.0).max(128.0), 14.0);
        let (rect, _) = ui.allocate_exact_size(desired, egui::Sense::hover());
        let painter = ui.painter_at(rect);
...
pub(super) fn gauge(ui: &mut egui::Ui, theme: DashboardTheme, spec: GaugeSpec<'_>) {
    let desired = egui::vec2(
        spec.width.clamp(124.0, 204.0),
        spec.height.clamp(96.0, 126.0),
    );
    let (rect, _) = ui.allocate_exact_size(desired, egui::Sense::hover());
    let painter = ui.painter_at(rect);
    ...
    let center = egui::pos2(rect.center().x, rect.bottom() - 18.0);
