...
pub(super) const STARTUP_FIT_MAX_WALL_TIME_S: f64 = 30.0;
...
const STARTUP_FIT_VERIFY_RPM_ERR: f64 = 35.0;
const STARTUP_FIT_VERIFY_NET_TORQUE_NM: f64 = 2.5;
const STARTUP_FIT_THROTTLE_BIN_COUNT: usize = 8;
const STARTUP_FIT_IGNITION_BIN_COUNT: usize = 8;
const STARTUP_FIT_MAX_CYCLES_PER_CANDIDATE: usize = 8;
...
const STARTUP_FIT_PERIODIC_ERROR_LIMIT: f64 = 0.08;
const STARTUP_FIT_CANDIDATE_RPM_ERR: f64 = 55.0;
const STARTUP_FIT_CANDIDATE_NET_TORQUE_NM: f64 = 4.0;
...
impl StartupFitPhase {
    ...
    pub(super) fn detail(self) -> &'static str {
        match self {
            ...
            Self::Verifying => {
                "applying the selected minimum-throttle candidate and checking that it holds the target speed on the live model"
            }
...
impl StartupFitState {
    ...
    pub(super) fn update(
        ...
        if self.phase != StartupFitPhase::Ready
            && self.started_wall_at.elapsed() >= Duration::from_secs_f64(STARTUP_FIT_MAX_WALL_TIME_S)
        {
    ...
    fn advance_search(&mut self, bounds: StartupFitControlBounds) {
        ...
        let candidate = match self.search_stage {
            SearchStage::Coarse => self.coarse_candidates.get(self.next_candidate_index).copied(),
            SearchStage::Refine => self.refine_candidates.get(self.next_candidate_index).copied(),
        };
        ...
        .clamp(bounds);
        let deadline = self.started_wall_at + Duration::from_secs_f64(STARTUP_FIT_MAX_WALL_TIME_S);
    ...
    fn update_best_result(&mut self, throttle_index: usize, result: CandidateResult) {
        ...
        let replace = target
            .map(|current| result.ranking_score(self.target_rpm) > current.ranking_score(self.target_rpm))
            .unwrap_or(true);
        if replace {
            *target = Some(result);
        }

        let improved = result.ranking_score(self.target_rpm)
            > CandidateResult {
                controls: self.best_controls,
                evaluation: CandidateEvaluation {
                    required_brake_torque_nm: self.best_required_brake_torque_nm,
                    ..self.last_evaluation
                },
            }
            .ranking_score(self.target_rpm);
        if improved || self.best_required_brake_torque_nm <= f64::EPSILON {
    ...
    fn select_final_candidate(&mut self) {
        let mut throttle_bests: Vec<CandidateResult> = self
            .throttle_tracks
            .iter()
            ...
            a.controls
                ...
                .then_with(|| {
                    b.evaluation
                        ...
                        .total_cmp(&a.evaluation.required_brake_torque_nm)
                })
        });
        ...
        let threshold = best_torque - STARTUP_FIT_TORQUE_MARGIN_FROM_BEST_NM;
        let selected = throttle_bests
            .iter()
            .copied()
            .find(|result| result.evaluation.required_brake_torque_nm >= threshold)
            .unwrap_or_else(|| {
                throttle_bests
                    .iter()
                    .copied()
                    .max_by(|a, b| {
                        a.ranking_score(self.target_rpm)
                            .total_cmp(&b.ranking_score(self.target_rpm))
                    })
                    .unwrap_or(throttle_bests[0])
            });
        self.torque_margin_to_best_nm = (best_torque - selected.evaluation.required_brake_torque_nm)
            .max(0.0);
    ...
    fn advance_verification(&mut self) {
        if self.verify_measure_elapsed_s < STARTUP_FIT_VERIFY_MEASURE_S || self.verify_sample_count == 0 {
            return;
        }
        ...
        let stable = (avg_rpm - self.target_rpm).abs() <= STARTUP_FIT_VERIFY_RPM_ERR
    ...
    fn finish_ready(&mut self, timed_out: bool) {
        ...
        self.current_controls = self.best_controls;
        if timed_out {
            self.last_candidate_label = "30 s cap reached / best candidate applied".to_owned();
        }
    }
    ...
    for _ in 0..STARTUP_FIT_MAX_STEPS_PER_CYCLE {
        ...
        loop {
            ...
            if error_norm <= 1.0 || try_dt_s <= STARTUP_FIT_MIN_DT_S * 1.01 {
                ...
                next_dt_s =
                    rkf45_next_dt(try_dt_s, error_norm).clamp(STARTUP_FIT_MIN_DT_S, sim.numerics.accuracy_dt_max_s);
                state = accepted;
                if crossed_cycle {
...
mod tests {
    ...
    use super::{
        STARTUP_FIT_INITIAL_THROTTLE, STARTUP_FIT_TARGET_RPM, SearchStage,
        STARTUP_FIT_TORQUE_MARGIN_FROM_BEST_NM, StartupFitControlBounds, StartupFitControls,
        StartupFitPhase, StartupFitState, linspace, rkf45_next_dt,
    };
